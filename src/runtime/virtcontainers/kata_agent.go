...
func (k *kataAgent) readTerminationMessage(ctx context.Context, c *Container) (string, error) {
	resp, err := k.sendReq(ctx, &grpc.ReadContainerTerminationMessageRequest{
		ContainerId: c.id,
	})
	if err != nil && err.Error() == context.DeadlineExceeded.Error() {
		return "", grpcStatus.Errorf(codes.DeadlineExceeded, "readTerminationMessage timed out")
	}
